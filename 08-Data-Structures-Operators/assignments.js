...
console.log(`Most likely to win: ${(team1 < team2 && 'Team 1') || 'Team 2'}`); // 7.
...
// 3.
const avgTimeEvent = Math.round(92 / gameEvents.size);
console.log(`An event happened, on average, every ${avgTimeEvent} minutes.`);

// 4.
for (const [key, value] of gameEvents) {
  const timeEvent = key < 45 ? 'FIRST' : 'SECOND';
  console.log(`[${timeEvent} HALF] ${key}: ${value}`);
}
...
document.querySelector('button').addEventListener('click', function () {
  ...
  for (const [i, line] of lines.entries()) {
    ...
    const convertedText = trimmedLine
      .toLowerCase()
      .replace(/([_][a-z])/g, match => match.toUpperCase().replace('_', ''));
    console.log(`${convertedText.padEnd(20)} ${'✅'.repeat(i + 1)}`);
  }
});
...
for (const f of flight) {
  let start = '';
  const [type, from, to, time] = f.replaceAll('_', ' ').trim().split(';');
  const [fromPlace, toPlace] = [
    from.slice(0, 3).toUpperCase(),
    to.slice(0, 3).toUpperCase(),
  ];
  const timeFormat = time.replace(':', 'h');
  start = type.startsWith('Delayed') ? '🔴' : start.padStart(12);
  console.log(
    `${start} ${type} from ${fromPlace} to ${toPlace} (${timeFormat})`
  );
}
